...
from utils_global import reorganize


class VecEnv(object):
    '''
    The wraper for a vector of environments. Here, only the
    basic interfaces of `env` are implemented. The vec environment
    does not support going backward in the doudizhu tree.
    '''
    ...
    def __init__(self, env_id, config):
        ...
        # Get the number of players/actions/state_shape in this doudizhu
...
def worker(remote, parent_remote, env_id, config):
    ...
    from env.registration import registry
