    ...
    def train(self):
        ...
        for i in range(self.epochs):
            ...
            for start in range(0,len(self.rewards), step_size):
                ...
                advantages = rewards - tf.reshape(values,-1) + gamma*tf.reshape(values_next, -1)*np.invert(done).astype(np.float32)
                ...
                with tf.GradientTape() as tape:
                    ...
                    action_log_probs_2 = tf.nn.softmax_cross_entropy_with_logits(legal_one_hot,probs) 
                    
                    actor_loss1 = action_log_probs * advantages 
                    actor_loss2 = - tf.reduce_mean(action_log_probs_2) * 0.1
                    actor_loss1 = - tf.reduce_mean(actor_loss1) 
                    
                    actor_loss = actor_loss2 + self.entropy_loss 
