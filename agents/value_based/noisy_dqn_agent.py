...
"""
double_dqn:  train independently two value networks:
one will be used to find the greedy action (the action with the maximal Q-value),
the other to estimate the Q-value itself.
"""
...
class NoisyDQNAgent:
    ...
    def __init__(self,
        ...
        self.local_net = DQNNoisy(state_shape=self.state_shape, num_actions=self.num_actions)
        self.target_net = DQNNoisy(state_shape=self.state_shape, num_actions=self.num_actions)
    ...
    def step(self, state):
        """
            ...
            NoisyNet does not need epsilon greedy.(i.e., self.epsilon = 0)
            Input:
                state (dict)
                    'obs' : actual state representation
                   'legal_actions' : possible legal actions to be taken from this state
